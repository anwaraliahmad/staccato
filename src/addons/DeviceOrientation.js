...
function DeviceOrientationControls ( object ) {
    ...
    this.update = function ({ theta = 0 } = { theta: 0 }) {
        ...
        if ( device ) {
            const alpha = device.alpha ? degToRad( device.alpha ) + scope.alphaOffset : 0; // Z
            
            const beta = device.beta ? degToRad( device.beta ) : 0; // X'
            const gamma = device.gamma ? degToRad( device.gamma ) : 0; // Y''
            const orient = scope.screenOrientation ? degToRad( scope.screenOrientation ) : 0; // O
    ...
    this.getAlpha = function() {
        ...
        return device && device.alpha ? device.alpha * Math.PI / 180 + scope.alphaOffset : 0;
    };
    this.getBeta = function() {
        const { deviceOrientation: device } = scope;
        return device && device.beta ? device.beta * Math.PI / 180 : 0;
    };
};
